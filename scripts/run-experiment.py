...
from dlmi.utils import create_dir, get_current_datetime, get_hostname, load_yaml, save_yaml,\
    get_environment_variable, increase_max_recursion_limit
...
if __name__ == '__main__':
    ...
    exp_name = f"{get_current_datetime()}-{job_id}-{config['Data']['dataset']}"
    ...
    db = Database(config, f'{dir_path}/index')
    partitions = \
        [100 * i for i in range(1, 10)] + \
        [1_000 * i for i in range(1, 10)] + \
        [10_000 * i for i in range(1, 10)] + \
        [100_000 * i for i in range(1, 10)] + \
        [1_000_000 * i for i in range(1, 11)]
    ...
    for i, p in enumerate(partitions):
        ...
        logging.info(f'Inserting paritions: {prev_p}-{p}')
        data_part = data.iloc[prev_p:p]
        info_df = db.insert(data_part, info_df)
        info_df.to_csv(f'{dir_path}/build/csv/build-{i}.csv', index=False)
        plot_build(info_df, f'{dir_path}/build/png/build-{i}.png', i)

        db.lmi.inconsistencies_stats().to_csv(f'{dir_path}/inconsistencies/inc-{i}.csv', index=False)
        db.lmi.dump_structure().to_csv(f'{dir_path}/struct/str-{i}.csv')

        stop_condition_leafs = list(set([
            int(db.lmi.get_n_leaf_nodes()*sc) for sc in config['Experiment']['stop_conditions'] \
            if int(db.lmi.get_n_leaf_nodes()*sc) > 0
        ]))
        ...
        for j, stop_condition_leaf in enumerate(stop_condition_leafs):
            results_knn, _ = get_knn_perf(
                ...
                data.iloc[:p],
                queries,
                metric=db.distance_function,
                k=100,
                stop_condition_leaf=stop_condition_leaf
            )
            results_knn['n-objects-total'] = db.lmi.get_n_of_objects()
        ...
        if len(stop_condition_leafs) != 0:
            plot_search(
                search_df,
                n_leaf_nodes=db.lmi.get_n_leaf_nodes(),
        ...
        search_df = pd.DataFrame([])
        stop_condition_times = db.config['Experiment']['stop_conditions_time']
        ...
        ### LEAF -- absoulte
        search_df = pd.DataFrame([])
        stop_condition_leafs = db.config['Experiment']['stop_conditions_absolute']
        for j, stop_condition_leaf in enumerate(stop_condition_leafs):
            if db.lmi.get_n_leaf_nodes() >= stop_condition_leaf:
