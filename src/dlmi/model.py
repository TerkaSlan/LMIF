...
class Model(nn.Module):
    def __init__(self, input_dim=4096, output_dim=10, model_type='MLP'):
        super().__init__()
        if model_type == 'MLP':
...
class NeuralNetwork():
    ...
    def predict(self, data_X: torch.FloatTensor):
        ...
        # Prevent 'RuntimeError: cannot perform reduction function max on tensor ...'
        except RuntimeError:
            y_pred = [0 for _ in range(len(self.model.output_neurons))]
            return np.array([self.model.output_neurons[label] for label in y_pred])
    ...
    def train(self, data_X: torch.FloatTensor, data_y: torch.LongTensor, epochs=500):
        losses = []
